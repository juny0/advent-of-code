...
for passport in passports:
    ...
    if "byr" in passport and "iyr" in passport and "eyr" in passport and "hgt" in passport and "hcl" in passport and "ecl" in passport and "pid" in passport:
        ...
        fParsed = passport.split()
        for kv in fParsed:
            kvSplit = kv.split(":")
            fields[kvSplit[0]] = kvSplit[1]
        ###### validate year information   
        if not (int(fields["byr"]) >= 1920 and int(fields["byr"]) <= 2002):
            continue
        if not (int(fields["iyr"]) >= 2010 and int(fields["iyr"]) <= 2020):
            continue 
        if not (int(fields["eyr"]) >= 2020 and int(fields["eyr"]) <= 2030):
            continue
        ###### validate height information
        heightFull = fields["hgt"]
        heightUnit = heightFull[len(heightFull)-2:len(heightFull)]
        if heightUnit == "cm" :
            h = int(heightFull[0:len(heightFull)-2])
        ...
        if not any(x == fields["ecl"] for x in validEyeColors):
            continue

        ###### validate passport ID 
        ppPattern = re.compile("[0-9]{9}")
        print(fields["pid"])
        bool = ppPattern.fullmatch(fields["pid"])
        print(bool)
        if not ppPattern.fullmatch(fields["pid"]):
            continue
        numValidPassports = numValidPassports + 1
