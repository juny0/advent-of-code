            ...
            for row in range(-1,2):
                for column in range(-1,2):
                    if row == 0 and column == 0:
                        continue
                    if i + row < 0 or i + row > len(input) - 1 or j + column < 0 or j + column > len(input[i]) - 1:
                        continue
                    adjSeat = input[i+row][j+column]
                    if adjSeat == "#":
                        adjacentOccupiedCount = adjacentOccupiedCount + 1
            # calculate if seat should be flipped
            valueToAppend = "."
            if currSeat == "L" and adjacentOccupiedCount == 0:
                valueToAppend = "#"
            elif currSeat == "#" and adjacentOccupiedCount >= 4:
                valueToAppend = "L"
            else:
