...
total = 0
bagsThatCanContainThisColor = {}
bagsThatCanContainShinyGold = {}
# make lookup dictionary for what color bags can contain a particular color
for rule in rules:
    rSplit = rule.split("contain")
    bagColor = rSplit[0].strip().split()
    bagColorParsed = bagColor[0] + " " + bagColor[1]
    allowedBags = rSplit[1].strip().replace(".","").split(",")
    for allowedBag in allowedBags:
        allowedBagSplit = allowedBag.split()
        allowedBagParsed = allowedBagSplit[1] + " " + allowedBagSplit[2]
        if not bagsThatCanContainThisColor.get(allowedBagParsed):
            bagsThatCanContainThisColor[allowedBagParsed] = {}
        bagsThatCanContainThisColor[allowedBagParsed][bagColorParsed] = True
    
#print(bagsThatCanContainThisColor)
colorsToCheck = ["shiny gold"]
while len(colorsToCheck) > 0:
    newColorsToCheck = []
    for colorToCheck in colorsToCheck:
        bagsThatContainCurrentColor = bagsThatCanContainThisColor.get(colorToCheck)
        if bagsThatContainCurrentColor:
            newColorsToCheck.extend(bagsThatContainCurrentColor.keys())
    print(newColorsToCheck)
    for n in newColorsToCheck:
        if not bagsThatCanContainShinyGold.get(n):
            bagsThatCanContainShinyGold[n] = True
    colorsToCheck = newColorsToCheck

print("How many bag colors can eventually contain at least one shiny gold bag?")
print(len(bagsThatCanContainShinyGold))
