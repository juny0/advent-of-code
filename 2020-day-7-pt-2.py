...
for rule in rules:
    ...
    for allowedBag in allowedBags:
        ...
        if not bagsContains.get(bagColorParsed):
            bagsContains[bagColorParsed] = {}
        if allowedBagSplit[0] != "no":
            bagsContains[bagColorParsed][allowedBagParsed] = int(allowedBagSplit[0])
...
while len(colorsToCheck) > 0:
    newColorsToCheck = {}
    for color in colorsToCheck.keys():
        numBags = colorsToCheck[color]
        mustContainedBags = bagsContains.get(color)
        if mustContainedBags:
            for bag in mustContainedBags.keys():
                totalBagCount = totalBagCount + (mustContainedBags[bag] * numBags)
                if not newColorsToCheck.get(bag):
                    newColorsToCheck[bag] = 0
                newColorsToCheck[bag] = newColorsToCheck[bag] + (mustContainedBags[bag] * numBags)
    colorsToCheck = newColorsToCheck
    print(colorsToCheck)
print("total bag count")
print(totalBagCount)
